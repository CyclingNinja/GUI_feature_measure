...
# define the measurements
    def length(self, frame):
        ...
        return (np.sqrt((B[0] - A[0])**2 + (B[1] - A[1])**2)*0.725*0.6)
    ...
    def width(self, frame):
        ...
        return (np.sqrt((C[0] - D[0])**2 + (C[1] - D[1])**2)*0.725*0.6)
...
#        if meas <= 90.0:
#            fi  = meas
#        elif 90.0 <= meas and meas <= 180.0:
#            fi = (meas - 90.0)*(-1)
#        elif 180.0 < meas and meas <= 270.0:
#            fi = (meas - 180.0)*(-1)
#        elif 270.0 < meas and meas <= 360.0:
#            fi = (meas - 270.0)
        return fi
    ...
    def vel(self, frame):
        if frame == 0:
            dels = ((self.length(frame + 1) - self.length(frame))*1000)
            ...
            dels = ((self.length(frame - 1) - self.length(frame + 1))*1000)
